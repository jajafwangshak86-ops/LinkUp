...
;; Post registry - key is plain uint post-id
(define-map posts
  uint
  {
    author:       principal,
...
;; Like registry - value is plain bool
(define-map likes
...
(define-public (like-post (post-id uint))
  (let ((post (unwrap! (map-get? posts post-id) ERR-NOT-FOUND)))
    ...
    (asserts! (is-none (map-get? likes { post-id: post-id, liker: tx-sender })) ERR-ALREADY-LIKED)
    (map-set likes { post-id: post-id, liker: tx-sender } true)
...
(define-public (tip-post (post-id uint) (amount uint))
  (let ((post (unwrap! (map-get? posts post-id) ERR-NOT-FOUND)))
    ...
    (map-set posts post-id (merge post { tips-total: (+ (get tips-total post) amount) }))
...
(define-public (delete-post (post-id uint))
  (let ((post (unwrap! (map-get? posts post-id) ERR-NOT-FOUND)))
    ...
    (map-set posts post-id (merge post { deleted: true }))
    (ok true)
  )
)
...
;; u200 - ERR-NOT-FOUND  u201 - ERR-ALREADY-LIKED  u202 - ERR-ZERO-TIP
;; u203 - ERR-SELF-TIP   u204 - ERR-NOT-AUTHOR      u205 - ERR-CONTENT-EMPTY
;; u206 - ERR-OVERFLOW
